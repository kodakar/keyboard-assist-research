...
Abdallahら\cite{abdallah2022light}はMediaPipe\cite{lugaresi2019mediapipe}とGRU/CNNを組み合わせた手話認識で99.84\%の精度を達成し，Leら\cite{le2023real}はYOLOv7による一人称視点からの手動作検出で95\%以上の精度を報告している．
...
これらの研究では，震えを信号処理により除去または補正するアプローチが主流となっている．
...
表\ref{tab:model_comparison}に示すように，
GRUはオフライン精度98.0\%，リアルタイム精度81.8\%と最も優れた性能を達成した．
また，LSTMと同等の長期依存関係学習能力を持ちながらパラメータ数が約24\%少なく学習・推論ともに高速であるため，
実用的なリアルタイム処理が可能である．
...
モデル & パラメータ数 & オフライン精度 & リアルタイム精度 \\\hline
LSTM & 約219K & 97.5\% & 72.7\% \\
CNN & 約169K & 93.6\% & 45.5\% \\
GRU & 約167K & \textbf{98.0\%} & \textbf{81.8\%} \\
TCN & 約209K & 94.2\% & 36.4\% \\\hline
\end{tabular}
\end{table}
...
各モデルの学習結果を表\ref{tab:model_evaluation}に示す．
