...
特に，震えを伴う入力では96.5\%で誤入力が発生したが，システムは64.9\%の試行で正しい意図を推定でき，
不随意運動下における入力補正の有効性が確認された．
...
キーボード入力への応用としては，Mallikら\cite{mallik2024virtual}がMediaPipe Holistic\cite{zhang2020mediapipe}とLong Short-Term Memory（LSTM）により仮想キーボードを実装し97\%の認識精度を達成，Rahimら\cite{rahimadvanced}は1D CNNとBi-GRUを組み合わせたCronoNetで多言語対応の仮想キーボードを開発した．
...
Wolkeら\cite{wolke2025validity}はMediaPipe\cite{lugaresi2019mediapipe}を用いた振戦の定量的評価手法を提案し，スマートフォンカメラでも臨床的に有効な精度が得られることを示した．
...
一方，時系列データからの運動意図推定では，LSTM\cite{hochreiter1997long}やGRU\cite{cho2014learning}などの再帰型ニューラルネットワークが広く用いられている．
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.9\columnwidth]{keyboard_mapping.png}
...
指先の軌跡は時間的に連続した動きであり，
震えを含む複雑な動作パターンから入力意図を推定するには時間的文脈を学習できるモデルが必要であるためである．
なお，LSTM\cite{hochreiter1997long}，
1次元畳み込みニューラルネットワーク（CNN），
Temporal Convolutional Network（TCN）\cite{bai2018empirical}との
比較検証については4.2節で後述する．
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.9\columnwidth]{data_collection_setup.png}
\caption{データ収集の実験環境}
...
\begin{figure}[tb]
\centering
\includegraphics[width=1.0\linewidth]{learning_curves.png}
\caption{GRUモデルの学習曲線}
\label{fig:learning_curves}
\end{figure}

次に，システムの基本動作を確認する予備実験として，
研究者本人が「hello world」を実際に入力するリアルタイム評価を行った．
...
GRUがTop-1精度100.0\%，Top-3精度100.0\%と最も優れた性能を示した．
LSTMもTop-1精度81.8\%，Top-3精度100.0\%と高い性能を達成したが，
GRUはオフライン評価（98.0\%）を上回る完璧な認識精度を実現した．
一方，CNNとTCNはTop-1精度がそれぞれ36.4\%，63.6\%にとどまり，
リアルタイム環境での性能が大幅に低下した．
この予備実験の結果に基づき，入力意図推定モデルとしてGRUを採用し，
次節の実用性評価を実施した．
...
LSTM & 81.8\% & 100.0\% & 43.6 \\
CNN & 36.4\% & 90.9\% & \textbf{47.7} \\
GRU & \textbf{100.0\%} & \textbf{100.0\%} & 41.8 \\
TCN & 63.6\% & 72.7\% & 45.5 \\\hline
\end{tabular}
\end{table}

\begin{figure}[tb]
\centering
\includegraphics[width=1.0\columnwidth]{realtime_prediction.png}
...
健常者10名を対象に評価実験を実施した．
評価には，言語モデルによる文脈予測の影響を排除するため，
37種類の全文字が均等に含まれるランダム文字列を入力タスクとして採用し，
システム単体の性能を測定した．
また，異なる入力状況下での性能を検証するため，
通常入力と振戦模擬入力の2種類の条件を設定した．
通常入力では，被験者に丁寧かつ正確な入力をするよう指示し，
標準的な使用状況下でのシステム性能を評価した．
振戦模擬入力では，目標キーに向かって指を動かし，
押下直前に手を瞬間的に動かして隣接する別のキーを押下するよう被験者に指示し，
振戦による誤入力を模擬した条件下でのシステムの頑健性を検証した．

表\ref{tab:usability}に被験者10名の平均性能を示す．
通常入力ではTop-1精度83.8\%，Top-3精度97.6\%を達成し，
システムがリアルタイムで動作することを確認した．
振戦模擬入力ではTop-1精度は64.9\%であったが，
本条件では被験者の操作により全試行の96.5\%で物理的な誤入力（隣接キー等の押下）が発生している．
したがって，このTop-1精度は実質的に，
誤入力が発生した際にシステムが自動的に正しい意図を推定できた割合（救済率65.0\%）を示している．
また，Top-3精度は90.0\%と高い値を維持しており，
ユーザーが誤ったキーを押下した場合でも，
9割の確率で提示候補から正解を選択可能であることが確認された．

\begin{table}[tb]
\caption{入力スタイル別性能評価（被験者10名の平均）}
\label{tab:usability}
\centering
\begin{tabular}{lcc}\hline\hline
入力スタイル & Top-1精度 & Top-3精度 \\\hline
通常入力 & 83.8\% & 97.6\% \\
振戦模擬入力 & 64.9\% & 90.0\% \\
\hline
\end{tabular}
...
特に，振戦を模擬した条件下においても誤入力に対する有効な救済率と高い候補推定精度を達成したことから，
...
GRUがオフライン精度98.0\%，リアルタイム精度100.0\%と最も優れた性能を示した．
LSTMもオフライン精度97.5\%，リアルタイム精度81.8\%と高精度を達成したが，
パラメータ数がGRUの約1.3倍（219K vs 167K）であり，
学習時間およびリアルタイム推論速度の面でGRUが優位であった．
CNNとTCNはオフライン精度がそれぞれ93.6\%，94.2\%にとどまり，
特にリアルタイム評価ではCNN 36.4\%，TCN 63.6\%と大幅に性能が低下した．
...
4.3節の実用性評価では，
通常入力と振戦模擬入力の2種類の条件で評価を実施した．
通常入力でTop-1精度83.8\%，Top-3精度97.6\%を達成し，
システムが実用的な入力速度において有効に機能することが確認された．

特筆すべきは，振戦模擬入力における救済効果である．
打鍵直前に手を瞬間的に動かすことで不随意運動を模擬した条件下では，
Top-1精度が64.9\%に低下したものの，
Top-3精度は90.0\%を維持した．
前述の通り，本条件では9割以上の入力で誤打鍵が発生しているため，
Top-1精度は実質的な自動補正率（救済率）とみなすことができる．
これは，ユーザーが誤ったキーを押下した場合でも，
半数以上の確率でシステムが即座に正解を入力できることを意味する．
さらに，Top-3精度が高いことから，
自動補正が外れた場合でも9割の確率で候補から正解を選択可能である．
運動機能障害者の支援においては，
この「自動補正」と「候補提示」の2段構えの救済機能が
入力効率を大幅に改善する可能性がある．

両条件においてTop-3精度がTop-1精度に比べて高い値を維持していることから，
...
\begin{figure}[tb]
\centering
\includegraphics[width=1.0\linewidth]{importance.png}
...
さらなる精度向上が期待できる．
特に，少量の個人データを用いた転移学習等の導入が有効と考えられる．
...
不随意運動を伴う軌跡から入力意図を推定する．

モデル比較実験では，LSTM，CNN，GRU，TCNの4種類を評価し，
GRUがオフライン精度98.0\%と最も優れた性能を示した．
実用性評価では，通常入力と振戦模擬入力の2条件で評価を実施し，
振戦を模擬した条件下においてもTop-1精度64.9\%（実質的な救済率），Top-3精度90.0\%を達成した．
これは，ユーザーが誤ったキーを押下した場合でも，
システムが高い確率で正しい入力意図を推定（自動補正または候補提示）できることを示しており，
